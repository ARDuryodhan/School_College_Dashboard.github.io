import React from "react";
import { Link } from "react-router-dom";

function PopularSearch() {
  return (
    <div className="Popular_container">
      <div className="popular_section">
        <h3>Popular Searches</h3>
        <div className="PopularSearchlayout">
          <div className="popular_box">
            <div className="popular_box_heading">
              <h4>
                <i className="fa fa-cog" aria-hidden="true"></i>
                <span>&nbsp;&nbsp;&nbsp;Engineering</span>
              </h4>
            </div>

            <article> 
              <ul>
                <li>
                  <Link to="/">
                    <i className="fa fa-angle-right" aria-hidden="true"></i>
                    Engineering Colleges in India
                  </Link>
                </li>
                <li>
                  <Link to="/">
                    <i className="fa fa-angle-right" aria-hidden="true"></i>
                    Engineering Colleges in Delhi
                  </Link>
                </li>
                <li>
                  <Link to="/">
                    <i className="fa fa-angle-right" aria-hidden="true"></i>
                    Engineering Colleges in Rajasthan
                  </Link>
                </li>
                <li>
                  <Link to="/">
                    <i className="fa fa-angle-right" aria-hidden="true"></i>
                    Engineering Colleges in Jaipur
                  </Link>
                </li>
                <li>
                  <Link to="/">
                    <i className="fa fa-angle-right" aria-hidden="true"></i>
                    Engineering Colleges in West Bengal
                  </Link>
                </li>
                <li>
                  <Link to="/">
                    <i className="fa fa-angle-right" aria-hidden="true"></i>
                    Engineering Colleges in Andhra
                  </Link>
                </li>
                <li>
                  <Link to="/">
                    <i className="fa fa-angle-right" aria-hidden="true"></i>
                    Engineering Colleges in Bangalore
                  </Link>
                </li>
                <li>
                  <Link to="/">
                    <i className="fa fa-angle-right" aria-hidden="true"></i>
                    Engineering Colleges in Gujarat
                  </Link>
                </li>
                <li>
                  <Link to="/">
                    <i className="fa fa-angle-right" aria-hidden="true"></i>
                    Engineering Colleges in Hyderabad
                  </Link>
                </li>
                <li>
                  <Link to="/">
                    <i className="fa fa-angle-right" aria-hidden="true"></i>
                    Engineering Colleges in Pune
                  </Link>
                </li>
                <li>
                  <Link to="/">
                    <i className="fa fa-angle-right" aria-hidden="true"></i>
                    Engineering Colleges in Chennai
                  </Link>
                </li>
                <li>
                  <Link to="/">
                    <i className="fa fa-angle-right" aria-hidden="true"></i>
                    Engineering Colleges in Gurgaon
                  </Link>
                </li>
                <li>
                  <Link to="/">
                    <i className="fa fa-angle-right" aria-hidden="true"></i>
                    Engineering Colleges in Kolkata
                  </Link>
                </li>
                <li>
                  <Link to="/">
                    <i className="fa fa-angle-right" aria-hidden="true"></i>
                    Engineering Colleges in Mumbai
                  </Link>
                </li>
                <li>
                  <Link to="/">
                    <i className="fa fa-angle-right" aria-hidden="true"></i>
                    Engineering Colleges in Noida
                  </Link>
                </li>
              </ul>
            </article>
          </div>

          <div className="popular_box">
            <div className="popular_box_heading">
              <h4>
                <i className="fa fa-graduation-cap" aria-hidden="true"></i>
                <span>&nbsp;&nbsp;&nbsp;MBA</span>
              </h4>
            </div>

            <article>
              <ul>
                <li>
                  <Link to="/">
                    <i className="fa fa-angle-right" aria-hidden="true"></i>MBA
                    Colleges in India
                  </Link>
                </li>
                <li>
                  <Link to="/">
                    <i className="fa fa-angle-right" aria-hidden="true"></i>MBA
                    Colleges in Mumbai
                  </Link>
                </li>
                <li>
                  <Link to="/">
                    <i className="fa fa-angle-right" aria-hidden="true"></i>MBA
                    Colleges in Pune
                  </Link>
                </li>
                <li>
                  <Link to="/">
                    <i className="fa fa-angle-right" aria-hidden="true"></i>MBA
                    Colleges in Ahmedabad
                  </Link>
                </li>
                <li>
                  <Link to="/">
                    <i className="fa fa-angle-right" aria-hidden="true"></i>MBA
                    Colleges in Bangalore
                  </Link>
                </li>
                <li>
                  <Link to="/">
                    <i className="fa fa-angle-right" aria-hidden="true"></i>MBA
                    Colleges in Chennai
                  </Link>
                </li>
                <li>
                  <Link to="/">
                    <i className="fa fa-angle-right" aria-hidden="true"></i>MBA
                    Colleges in Delhi
                  </Link>
                </li>
                <li>
                  <Link to="/">
                    <i className="fa fa-angle-right" aria-hidden="true"></i>MBA
                    Colleges in Rajasthan
                  </Link>
                </li>
                <li>
                  <Link to="/">
                    <i className="fa fa-angle-right" aria-hidden="true"></i>MBA
                    Colleges in Jaipur
                  </Link>
                </li>
                <li>
                  <Link to="/">
                    <i className="fa fa-angle-right" aria-hidden="true"></i>MBA
                    Colleges in Kolkata
                  </Link>
                </li>
                <li>
                  <Link to="/">
                    <i className="fa fa-angle-right" aria-hidden="true"></i>MBA
                    Colleges in Kerala
                  </Link>
                </li>
                <li>
                  <Link to="/">
                    <i className="fa fa-angle-right" aria-hidden="true"></i>MBA
                    Colleges in Indore
                  </Link>
                </li>
                <li>
                  <Link to="/">
                    <i className="fa fa-angle-right" aria-hidden="true"></i>MBA
                    Colleges in Himachal Pradesh
                  </Link>
                </li>
                <li>
                  <Link to="/">
                    <i className="fa fa-angle-right" aria-hidden="true"></i>MBA
                    Colleges in Assam
                  </Link>
                </li>
                <li>
                  <Link to="/">
                    <i className="fa fa-angle-right" aria-hidden="true"></i>MBA
                    Colleges in Goa
                  </Link>
                </li>
              </ul>
            </article>
          </div>

          <div className="popular_box">
            <div className="popular_box_heading">
              <h4>
                <i className="fa fa-stethoscope" aria-hidden="true"></i>
                <span>&nbsp;&nbsp;&nbsp;Medical</span>
              </h4>
            </div>

            <article>
              <ul>
                <li>
                  <Link to="/">
                    <i className="fa fa-angle-right" aria-hidden="true"></i>
                    Medical Colleges in India
                  </Link>
                </li>
                <li>
                  <Link to="/">
                    <i className="fa fa-angle-right" aria-hidden="true"></i>
                    Medical Colleges in Bangalore
                  </Link>
                </li>
                <li>
                  <Link to="/">
                    <i className="fa fa-angle-right" aria-hidden="true"></i>
                    Medical Colleges in Mumbai
                  </Link>
                </li>
                <li>
                  <Link to="/">
                    <i className="fa fa-angle-right" aria-hidden="true"></i>
                    Medical Colleges in Karnataka
                  </Link>
                </li>
                <li>
                  <Link to="/">
                    <i className="fa fa-angle-right" aria-hidden="true"></i>
                    Medical Colleges in Maharashtra
                  </Link>
                </li>
                <li>
                  <Link to="/">
                    <i className="fa fa-angle-right" aria-hidden="true"></i>
                    Medical Colleges in Delhi
                  </Link>
                </li>
                <li>
                  <Link to="/">
                    <i className="fa fa-angle-right" aria-hidden="true"></i>
                    Medical Colleges in Pune
                  </Link>
                </li>
                <li>
                  <Link to="/">
                    <i className="fa fa-angle-right" aria-hidden="true"></i>
                    Medical Colleges in Chennai
                  </Link>
                </li>
                <li>
                  <Link to="/">
                    <i className="fa fa-angle-right" aria-hidden="true"></i>
                    Medical Colleges in Kerala
                  </Link>
                </li>
                <li>
                  <Link to="/">
                    <i className="fa fa-angle-right" aria-hidden="true"></i>
                    Medical Colleges in Punjab
                  </Link>
                </li>
                <li>
                  <Link to="/">
                    <i className="fa fa-angle-right" aria-hidden="true"></i>
                    Medical Colleges in Gujarat
                  </Link>
                </li>
                <li>
                  <Link to="/">
                    <i className="fa fa-angle-right" aria-hidden="true"></i>
                    Medical Colleges in Bihar
                  </Link>
                </li>
                <li>
                  <Link to="/">
                    <i className="fa fa-angle-right" aria-hidden="true"></i>
                    Medical Colleges in Chhattisgarh
                  </Link>
                </li>
                <li>
                  <Link to="/">
                    <i className="fa fa-angle-right" aria-hidden="true"></i>
                    Medical Colleges in Haryana
                  </Link>
                </li>
                <li>
                  <Link to="/">
                    <i className="fa fa-angle-right" aria-hidden="true"></i>
                    Medical Colleges in Meghalaya
                  </Link>
                </li>
              </ul>
            </article>
          </div>

          <div className="popular_box">
            <div className="popular_box_heading">
              <h4>
                <i className="fa fa-graduation-cap" aria-hidden="true"></i>
                <span>&nbsp;&nbsp;&nbsp;BBA </span>
              </h4>
            </div>

            <article>
              <ul>
                <li>
                  <Link to="/">
                    <i className="fa fa-angle-right" aria-hidden="true"></i> BBA
                    Colleges in India
                  </Link>
                </li>
                <li>
                  <Link to="/">
                    <i className="fa fa-angle-right" aria-hidden="true"></i> BBA
                    Colleges in Delhi
                  </Link>
                </li>
                <li>
                  <Link to="/">
                    <i className="fa fa-angle-right" aria-hidden="true"></i>BBA
                    Colleges in Mumbai
                  </Link>
                </li>
                <li>
                  <Link to="/">
                    <i className="fa fa-angle-right" aria-hidden="true"></i>BBA
                    Colleges in Kolkata
                  </Link>
                </li>
                <li>
                  <Link to="/">
                    <i className="fa fa-angle-right" aria-hidden="true"></i>BBA
                    Colleges in Pune
                  </Link>
                </li>
                <li>
                  <Link to="/">
                    <i className="fa fa-angle-right" aria-hidden="true"></i>BBA
                    Colleges in Hyderabad
                  </Link>
                </li>
                <li>
                  <Link to="/">
                    <i className="fa fa-angle-right" aria-hidden="true"></i>BBA
                    Colleges in Rajasthan
                  </Link>
                </li>
                <li>
                  <Link to="/">
                    <i className="fa fa-angle-right" aria-hidden="true"></i>BBA
                    Colleges in Uttar Pradesh
                  </Link>
                </li>
                <li>
                  <Link to="/">
                    <i className="fa fa-angle-right" aria-hidden="true"></i>BBA
                    Colleges in Gujarat
                  </Link>
                </li>
                <li>
                  <Link to="/">
                    <i className="fa fa-angle-right" aria-hidden="true"></i>BBA
                    Colleges in Chennai
                  </Link>
                </li>
                <li>
                  <Link to="/">
                    <i className="fa fa-angle-right" aria-hidden="true"></i>BBA
                    Colleges in Bangalore
                  </Link>
                </li>
                <li>
                  <Link to="/">
                    <i className="fa fa-angle-right" aria-hidden="true"></i>BBA
                    Colleges in Chhattisgarh
                  </Link>
                </li>
                <li>
                  <Link to="/">
                    <i className="fa fa-angle-right" aria-hidden="true"></i>BBA
                    Colleges in Jharkhand
                  </Link>
                </li>
                <li>
                  <Link to="/">
                    <i className="fa fa-angle-right" aria-hidden="true"></i>BBA
                    Colleges in Telangana
                  </Link>
                </li>
                <li>
                  <Link to="/">
                    <i className="fa fa-angle-right" aria-hidden="true"></i>BBA
                    Colleges in West Bengal
                  </Link>
                </li>
              </ul>
            </article>
          </div>

          <div className="popular_box">
            <div className="popular_box_heading">
              <h4>
                <i className="fa-solid fa-gavel"></i>
                <span>&nbsp;&nbsp;&nbsp;LAW</span>
              </h4>
            </div>

            <article>
              <ul>
                <li>
                  <Link to="/">
                    <i className="fa fa-angle-right" aria-hidden="true"></i>
                    Law Colleges in India
                  </Link>
                </li>
                <li>
                  <Link to="/">
                    <i className="fa fa-angle-right" aria-hidden="true"></i>
                    Law Colleges in Delhi
                  </Link>
                </li>
                <li>
                  <Link to="/">
                    <i className="fa fa-angle-right" aria-hidden="true"></i>
                    Law Colleges in Rajasthan
                  </Link>
                </li>
                <li>
                  <Link to="/">
                    <i className="fa fa-angle-right" aria-hidden="true"></i>
                    Law Colleges in Jaipur
                  </Link>
                </li>
                <li>
                  <Link to="/">
                    <i className="fa fa-angle-right" aria-hidden="true"></i>
                    Law Colleges in West Bengal
                  </Link>
                </li>
                <li>
                  <Link to="/">
                    <i className="fa fa-angle-right" aria-hidden="true"></i>
                    Law Colleges in Andhra Pradesh
                  </Link>
                </li>
                <li>
                  <Link to="/">
                    <i className="fa fa-angle-right" aria-hidden="true"></i>
                    Law Colleges in Bangalore
                  </Link>
                </li>
                <li>
                  <Link to="/">
                    <i className="fa fa-angle-right" aria-hidden="true"></i>
                    Law Colleges in Gujarat
                  </Link>
                </li>
                <li>
                  <Link to="/">
                    <i className="fa fa-angle-right" aria-hidden="true"></i>
                    Law Colleges in Hyderabad
                  </Link>
                </li>
                <li>
                  <Link to="/">
                    <i className="fa fa-angle-right" aria-hidden="true"></i>
                    Law Colleges in Pune
                  </Link>
                </li>
                <li>
                  <Link to="/">
                    <i className="fa fa-angle-right" aria-hidden="true"></i>
                    Law Colleges in Chennai
                  </Link>
                </li>
                <li>
                  <Link to="/">
                    <i className="fa fa-angle-right" aria-hidden="true"></i>
                    Law Colleges in Gurgaon
                  </Link>
                </li>
                <li>
                  <Link to="/">
                    <i className="fa fa-angle-right" aria-hidden="true"></i>
                    Law Colleges in Kolkata
                  </Link>
                </li>
                <li>
                  <Link to="/">
                    <i className="fa fa-angle-right" aria-hidden="true"></i>
                    Law Colleges in Mumbai
                  </Link>
                </li>
                <li>
                  <Link to="/">
                    <i className="fa fa-angle-right" aria-hidden="true"></i>
                    Law Colleges in Noida
                  </Link>
                </li>
              </ul>
            </article>
          </div>

          <div className="popular_box">
            <div className="popular_box_heading">
              <h4>
              <i className="fa-solid fa-book"></i>
                <span>&nbsp;&nbsp;&nbsp;Arts</span>
              </h4>
            </div>

            <article>
              <ul>
                <li>
                  <Link to="/">
                    <i className="fa fa-angle-right" aria-hidden="true"></i>
                    Arts Colleges in India
                  </Link>
                </li>
                <li>
                  <Link to="/">
                    <i className="fa fa-angle-right" aria-hidden="true"></i>
                    Arts Colleges in Delhi
                  </Link>
                </li>
                <li>
                  <Link to="/">
                    <i className="fa fa-angle-right" aria-hidden="true"></i>
                    Arts Colleges in Rajasthan
                  </Link>
                </li>
                <li>
                  <Link to="/">
                    <i className="fa fa-angle-right" aria-hidden="true"></i>
                    Arts Colleges in Jaipur
                  </Link>
                </li>
                <li>
                  <Link to="/">
                    <i className="fa fa-angle-right" aria-hidden="true"></i>
                    Arts Colleges in West Bengal
                  </Link>
                </li>
                <li>
                  <Link to="/">
                    <i className="fa fa-angle-right" aria-hidden="true"></i>
                    Arts Colleges in Andhra
                  </Link>
                </li>
                <li>
                  <Link to="/">
                    <i className="fa fa-angle-right" aria-hidden="true"></i>
                    Arts Colleges in Bangalore
                  </Link>
                </li>
                <li>
                  <Link to="/">
                    <i className="fa fa-angle-right" aria-hidden="true"></i>
                    Arts Colleges in Gujarat
                  </Link>
                </li>
                <li>
                  <Link to="/">
                    <i className="fa fa-angle-right" aria-hidden="true"></i>
                    Arts Colleges in Hyderabad
                  </Link>
                </li>
                <li>
                  <Link to="/">
                    <i className="fa fa-angle-right" aria-hidden="true"></i>
                    Arts Colleges in Pune
                  </Link>
                </li>
                <li>
                  <Link to="/">
                    <i className="fa fa-angle-right" aria-hidden="true"></i>
                    Arts Colleges in Chennai
                  </Link>
                </li>
                <li>
                  <Link to="/">
                    <i className="fa fa-angle-right" aria-hidden="true"></i>
                    Arts Colleges in Gurgaon
                  </Link>
                </li>
                <li>
                  <Link to="/">
                    <i className="fa fa-angle-right" aria-hidden="true"></i>
                    Arts Colleges in Kolkata
                  </Link>
                </li>
                <li>
                  <Link to="/">
                    <i className="fa fa-angle-right" aria-hidden="true"></i>
                    Arts Colleges in Mumbai
                  </Link>
                </li>
                <li>
                  <Link to="/">
                    <i className="fa fa-angle-right" aria-hidden="true"></i>
                    Arts Colleges in Noida
                  </Link>
                </li>
              </ul>
            </article>
          </div>

          <div className="popular_box">
            <div className="popular_box_heading">
              <h4>
              <i className="fa-solid fa-capsules"></i>
                <span>&nbsp;&nbsp;&nbsp;Pharmacy</span>
              </h4>
            </div>

            <article>
              <ul>
                <li>
                  <Link to="/">
                    <i className="fa fa-angle-right" aria-hidden="true"></i>
                    Pharmacy Colleges in India
                  </Link>
                </li>
                <li>
                  <Link to="/">
                    <i className="fa fa-angle-right" aria-hidden="true"></i>
                    Pharmacy Colleges in Delhi
                  </Link>
                </li>
                <li>
                  <Link to="/">
                    <i className="fa fa-angle-right" aria-hidden="true"></i>
                    Pharmacy Colleges in Rajasthan
                  </Link>
                </li>
                <li>
                  <Link to="/">
                    <i className="fa fa-angle-right" aria-hidden="true"></i>
                    Pharmacy Colleges in Jaipur
                  </Link>
                </li>
                <li>
                  <Link to="/">
                    <i className="fa fa-angle-right" aria-hidden="true"></i>
                    Pharmacy Colleges in West Bengal
                  </Link>
                </li>
                <li>
                  <Link to="/">
                    <i className="fa fa-angle-right" aria-hidden="true"></i>
                    Pharmacy Colleges in Andhra
                  </Link>
                </li>
                <li>
                  <Link to="/">
                    <i className="fa fa-angle-right" aria-hidden="true"></i>
                    Pharmacy Colleges in Bangalore
                  </Link>
                </li>
                <li>
                  <Link to="/">
                    <i className="fa fa-angle-right" aria-hidden="true"></i>
                    Pharmacy Colleges in Gujarat
                  </Link>
                </li>
                <li>
                  <Link to="/">
                    <i className="fa fa-angle-right" aria-hidden="true"></i>
                    Pharmacy Colleges in Hyderabad
                  </Link>
                </li>
                <li>
                  <Link to="/">
                    <i className="fa fa-angle-right" aria-hidden="true"></i>
                    Pharmacy Colleges in Pune
                  </Link>
                </li>
                <li>
                  <Link to="/">
                    <i className="fa fa-angle-right" aria-hidden="true"></i>
                    Pharmacy Colleges in Chennai
                  </Link>
                </li>
                <li>
                  <Link to="/">
                    <i className="fa fa-angle-right" aria-hidden="true"></i>
                    Pharmacy Colleges in Gurgaon
                  </Link>
                </li>
                <li>
                  <Link to="/">
                    <i className="fa fa-angle-right" aria-hidden="true"></i>
                    Pharmacy Colleges in Kolkata
                  </Link>
                </li>
                <li>
                  <Link to="/">
                    <i className="fa fa-angle-right" aria-hidden="true"></i>
                    Pharmacy Colleges in Mumbai
                  </Link>
                </li>
                <li>
                  <Link to="/">
                    <i className="fa fa-angle-right" aria-hidden="true"></i>
                    Pharmacy Colleges in Noida
                  </Link>
                </li>
              </ul>
            </article>
          </div>

          <div className="popular_box">
            <div className="popular_box_heading">
              <h4>
                <i className="fa fa-cog" aria-hidden="true"></i>
                <span>&nbsp;&nbsp;&nbsp;Design</span>
              </h4>
            </div>

            <article>
              <ul>
                <li>
                  <Link to="/">
                    <i className="fa fa-angle-right" aria-hidden="true"></i>
                    Design Colleges in India
                  </Link>
                </li>
                <li>
                  <Link to="/">
                    <i className="fa fa-angle-right" aria-hidden="true"></i>
                    Design Colleges in Delhi
                  </Link>
                </li>
                <li>
                  <Link to="/">
                    <i className="fa fa-angle-right" aria-hidden="true"></i>
                    Design Colleges in Rajasthan
                  </Link>
                </li>
                <li>
                  <Link to="/">
                    <i className="fa fa-angle-right" aria-hidden="true"></i>
                    Design Colleges in Jaipur
                  </Link>
                </li>
                <li>
                  <Link to="/">
                    <i className="fa fa-angle-right" aria-hidden="true"></i>
                    Design Colleges in West Bengal
                  </Link>
                </li>
                <li>
                  <Link to="/">
                    <i className="fa fa-angle-right" aria-hidden="true"></i>
                    Design Colleges in Andhra
                  </Link>
                </li>
                <li>
                  <Link to="/">
                    <i className="fa fa-angle-right" aria-hidden="true"></i>
                    Design Colleges in Bangalore
                  </Link>
                </li>
                <li>
                  <Link to="/">
                    <i className="fa fa-angle-right" aria-hidden="true"></i>
                    Design Colleges in Gujarat
                  </Link>
                </li>
                <li>
                  <Link to="/">
                    <i className="fa fa-angle-right" aria-hidden="true"></i>
                    Design Colleges in Hyderabad
                  </Link>
                </li>
                <li>
                  <Link to="/">
                    <i className="fa fa-angle-right" aria-hidden="true"></i>
                    Design Colleges in Pune
                  </Link>
                </li>
                <li>
                  <Link to="/">
                    <i className="fa fa-angle-right" aria-hidden="true"></i>
                    Design Colleges in Chennai
                  </Link>
                </li>
                <li>
                  <Link to="/">
                    <i className="fa fa-angle-right" aria-hidden="true"></i>
                    Design Colleges in Gurgaon
                  </Link>
                </li>
                <li>
                  <Link to="/">
                    <i className="fa fa-angle-right" aria-hidden="true"></i>
                    Design Colleges in Kolkata
                  </Link>
                </li>
                <li>
                  <Link to="/">
                    <i className="fa fa-angle-right" aria-hidden="true"></i>
                    Design Colleges in Mumbai
                  </Link>
                </li>
                <li>
                  <Link to="/">
                    <i className="fa fa-angle-right" aria-hidden="true"></i>
                    Design Colleges in Noida
                  </Link>
                </li>
              </ul>
            </article>
          </div>
        </div>
      </div>
    </div>
  );
}

export default PopularSearch;
