import React from "react";

import findsclgBanner from "../../assets/findclgimg.jpg";
function findcollege() {
  return (
    <div className="Findpage_container" style={{ backgroundColor: "white" }}>
      <div className="blackbox">
        <div className="findschoolBaner">
          <img src={findsclgBanner} alt="findschool" />
        </div>
      </div>
      <div className="findclgTitle">
        <p style={{ color: "white", textShadow: "1px 1px 2px black" }}>
          Find a best Colleges
        </p>
      </div>
      <div className="findschl_statecard"></div>
      <h3>Browse by Category</h3>
      <div className="findschl_cards">
        <div className="row">
          <h4>Top college by Course Category</h4>
          <div className="row cards">
            <div className="col">
              <ul>
                <li className="siw1">
                  <i className="fa-solid fa-check"></i>
                  Management college in India
                </li>
                <li className="siw1">
                  <i className="fa-solid fa-check"></i>
                  Engineering college in India
                </li>
                <li className="siw1">
                  <i className="fa-solid fa-check"></i>
                  Medical college in India
                </li>
                <li className="siw1">
                  <i className="fa-solid fa-check"></i>
                  Hotel Management college in India
                </li>
                <li className="siw1">
                  <i className="fa-solid fa-check"></i>
                  Finance and Accounts college in India
                </li>
              </ul>
            </div>
            <div className="col">
              <ul>
                <li className="siw1">
                  <i className="fa-solid fa-check"></i>
                  Arts college in India
                </li>
                <li className="siw1">
                  <i className="fa-solid fa-check"></i>
                  Law college in India
                </li>
                <li className="siw1">
                  <i className="fa-solid fa-check"></i>
                  Films Making &amp; Journalism college in India
                </li>
                <li className="siw1">
                  <i className="fa-solid fa-check"></i>
                  Computer Applications college in India
                </li>
                <li className="siw1">
                  <i className="fa-solid fa-check"></i>
                  Animation college in India
                </li>
              </ul>
            </div>
          </div>
        </div>
        <div className="row">
          <h4>State Wise Medical College in India</h4>
          <div className="row cards">
            <div className="col">
              <ul>
                <li className="siw2">
                  <i className="fa-solid fa-check"></i>
                  Medical College in Andhra Pradesh
                </li>

                <li className="siw2">
                  <i className="fa-solid fa-check"></i>
                  Medical College in Arunachal Pradesh
                </li>

                <li className="siw2">
                  <i className="fa-solid fa-check"></i>
                  Medical College in Assam
                </li>

                <li className="siw2">
                  <i className="fa-solid fa-check"></i>
                  Medical College in Bihar
                </li>

                <li className="siw2">
                  <i className="fa-solid fa-check"></i>
                  Medical College in Chhattisgarh
                </li>
              </ul>
            </div>
            <div className="col">
              <ul>
                <li className="siw2">
                  <i className="fa-solid fa-check"></i>
                  Medical College in Delhi
                </li>

                <li className="siw2">
                  <i className="fa-solid fa-check"></i>
                  Medical College in Delhi NCR
                </li>

                <li className="siw2">
                  <i className="fa-solid fa-check"></i>
                  Medical College in Goa
                </li>

                <li className="siw2">
                  <i className="fa-solid fa-check"></i>
                  Medical College in Gujarat
                </li>

                <li className="siw2">
                  <i className="fa-solid fa-check"></i>
                  Medical College in Haryana
                </li>
              </ul>
            </div>
          </div>
        </div>

        <div className="row">
          <h4>State Wise Engineering College in India</h4>
          <div className="row cards">
            <div className="col">
              <ul>
                <li className="siw2">
                  <i className="fa-solid fa-check"></i>
                  Engineering College in Andhra Pradesh
                </li>
                <li className="siw2">
                  <i className="fa-solid fa-check"></i>
                  Engineering College in Arunachal Pradesh
                </li>
                <li className="siw2">
                  <i className="fa-solid fa-check"></i>
                  Engineering College in Assam
                </li>
                <li className="siw2">
                  <i className="fa-solid fa-check"></i>
                  Engineering College in Bihar
                </li>
                <li className="siw2">
                  <i className="fa-solid fa-check"></i>
                  Engineering College in Chhattisgarh
                </li>
              </ul>
            </div>
            <div className="col">
              <ul>
                <li className="siw2">
                  <i className="fa-solid fa-check"></i>
                  Engineering College in Delhi
                </li>
                <li className="siw2">
                  <i className="fa-solid fa-check"></i>
                  Engineering College in Delhi NCR
                </li>
                <li className="siw2">
                  <i className="fa-solid fa-check"></i>
                  Engineering College in Goa
                </li>
                <li className="siw2">
                  <i className="fa-solid fa-check"></i>
                  Engineering College in Gujarat
                </li>
                <li className="siw2">
                  <i className="fa-solid fa-check"></i>
                  Engineering College in Haryana
                </li>
              </ul>
            </div>
          </div>
        </div>

        <div className="row">
          <h4>Top Management College in India</h4>
          <div className="row cards">
            <div className="col">
              <ul>
                <li className="siw1">
                  <i className="fa-solid fa-check"></i>
                  Krishna Institute of Management
                </li>
                <li className="siw1">
                  <i className="fa-solid fa-check"></i>
                  Kishan Institute Of Information Technology
                </li>
                <li className="siw1">
                  <i className="fa-solid fa-check"></i>
                  Institute of Productivity &amp; Management
                </li>
                <li className="siw1">
                  <i className="fa-solid fa-check"></i>
                  Translam Institute of Technology and Management
                </li>
                <li className="siw1">
                  <i className="fa-solid fa-check"></i>
                  Vidya College Of Engineering
                </li>
              </ul>
            </div>
            <div className="col">
              <ul>
                <li className="siw1">
                  <i className="fa-solid fa-check"></i>
                  School of Business Studies, Shobhit University
                </li>
                <li className="siw1">
                  <i className="fa-solid fa-check"></i>
                  Dewan Institute of Management Studies
                </li>
                <li className="siw1">
                  <i className="fa-solid fa-check"></i>
                  Apex College of Technical Education
                </li>
                <li className="siw1">
                  <i className="fa-solid fa-check"></i>
                  Bharat Institute of Technology
                </li>
                <li className="siw1">
                  <i className="fa-solid fa-check"></i>
                  Rishi Institute of Engineering &amp; Technology
                </li>
              </ul>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
}

export default findcollege;
